...
class CBFM(nn.Module):    # all use group conv
    def __init__(self, group = 1,in_ch=512):
        ...
        self.cf1  = nn.Sequential(
            nn.Conv2d(in_ch, int(in_ch/10), kernel_size=3, stride=1, padding=1, bias=True),
    ...
    def forward(self,cf,gf):
        ...
        out_gf = self.gfout(gf+torch.cat(cfout,dim=1))
        # out_gf = self.gfout(CConcat(gf,torch.cat(cfout,dim=1),self.group))
        cf = self.cfout(self.xx3(self.se(gf))+cf)
        return cf,out_gf
...
class Block(nn.Module):
    ...
    def __init__(self, dim=441, num_heads=9, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
...
class DBANet(nn.Module):    # all use group conv
    ...
    def __init__(self, group = 1):
        ...
        self.Conv = nn.Sequential(
            nn.Conv2d(filters[0]*2, filters[0], kernel_size=3, stride=1, padding=1),
