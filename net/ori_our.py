...
class DSPP(nn.Module):
    def __init__(self, in_channels,out_ch=256, split=[1,1,2,4,8],atrous_rates=[6, 12, 18]):
        ...
        modules.append(nn.Sequential(
            nn.Conv2d(out_channels[0], out_channels[0], 1, bias=False),
            nn.BatchNorm2d(out_channels[0]),
            nn.ReLU(inplace=True)))

        rate1, rate2, rate3 = tuple(atrous_rates)
        modules.append(ASPPConv(out_channels[1], out_channels[1], rate1))
        modules.append(ASPPConv(out_channels[2], out_channels[2], rate2))
        modules.append(ASPPConv(out_channels[3], out_channels[3], rate3))
        modules.append(ASPPPooling(out_channels[4], out_channels[4]))
...
class MSTB(nn.Module):
    def __init__(self,in_ch,out_ch,down_ratio=1,out_format='conv',need_up=True):
...
class SFFNet(nn.Module):
    ...
    def forward(self, x):
        ...
        e5 = torch.cat([e3, e5], dim=1) # [bs,384,84,84]
